...
*Setup instructions for a gitian build of Spoomy using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Spoomy
...
- [Building Spoomy](#building-spoomy)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-spoomy-host/README.md).
...
- Storage on Physical hard drive: Spoomyally Allocated 
...
Clone the git repositories for spoomy and gitian and then checkout the spoomy version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the spoomy repository
...
To build Spoomy (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the spoomy repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/spoomy/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/spoomy-zmy/spoomy
...
For example:
```bash
URL=https://github.com/crowning-/spoomy.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit spoomy=${COMMIT} --url spoomy=${URL} ../spoomy/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit spoomy=${COMMIT} --url spoomy=${URL} ../spoomy/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit spoomy=${COMMIT} --url spoomy=${URL} ../spoomy/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/spoomy-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/spoomy-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/spoomy-build.assert
```
...
In the future it will be possible to push your signatures (both the `.assert` and `.assert.sig` files). There will be an official announcement when this repository is online.
