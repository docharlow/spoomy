...
UniValue importprivkey(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw std::runtime_error(
            "importprivkey \"zumyprivkey\" ( \"label\" rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"zumyprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
void ImportAddress(const CSpoomyAddress& address, const std::string& strLabel);
void ImportScript(const CScript& script, const std::string& strLabel, bool isRedeemScript)
{
    ...
    if (isRedeemScript) {
        ...
        ImportAddress(CSpoomyAddress(CScriptID(script)), strLabel);
    }
}
...
UniValue importaddress(const UniValue& params, bool fHelp)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Spoomy address or script");
    }
...
UniValue importpubkey(const UniValue& params, bool fHelp)
{
    ...
    ImportAddress(CSpoomyAddress(pubKey.GetID()), strLabel);
...
UniValue dumpprivkey(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            "dumpprivkey \"zumyaddress\"\n"
            "\nReveals the private key corresponding to 'zumyaddress'.\n"
            "Then the importprivkey can be used with this output\n"
            "\nArguments:\n"
            "1. \"zumyaddress\"   (string, required) The spoomy address for the private key\n"
            "\nResult:\n"
    ...
    return CSpoomySecret(vchSecret).ToString();
}
...
for (std::vector<std::pair<int64_t, CKeyID> >::const_iterator it = vKeyBirth.begin(); it != vKeyBirth.end(); it++) {
        ...
        std::string strAddr = CSpoomyAddress(keyid).ToString();
        CKey key;
        if (pwalletMain->GetKey(keyid, key)) {
            file << strprintf("%s %s ", CSpoomySecret(key).ToString(), strTime);
