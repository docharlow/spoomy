...
UniValue getnewaddress(const UniValue& params, bool fHelp)
{
    ...
    return CSpoomyAddress(keyID).ToString();
}


CSpoomyAddress GetAccountAddress(std::string strAccount, bool bForceNew=false)
{
    ...
    return CSpoomyAddress(account.vchPubKey.GetID());
}
...
UniValue setaccount(const UniValue& params, bool fHelp)
{
    ...
    CSpoomyAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Spoomy address");
...
UniValue getaddressesbyaccount(const UniValue& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const PAIRTYPE(CSpoomyAddress, CAddressBookData)& item, pwalletMain->mapAddressBook)
    {
        const CSpoomyAddress& address = item.first;
...
UniValue listaddressgroupings(const UniValue& params, bool fHelp)
{
    ...
    BOOST_FOREACH(std::set<CTxDestination> grouping, pwalletMain->GetAddressGroupings())
    {
        ...
        BOOST_FOREACH(CTxDestination address, grouping)
        {
            ...
            addressInfo.push_back(CSpoomyAddress(address).ToString());
            addressInfo.push_back(ValueFromAmount(balances[address]));
            {
                if (pwalletMain->mapAddressBook.find(CSpoomyAddress(address).Get()) != pwalletMain->mapAddressBook.end())
                    addressInfo.push_back(pwalletMain->mapAddressBook.find(CSpoomyAddress(address).Get())->second.name);
            }
...
UniValue signmessage(const UniValue& params, bool fHelp)
{
    ...
    CSpoomyAddress addr(strAddress);
    if (!addr.IsValid())
...
UniValue getreceivedbyaddress(const UniValue& params, bool fHelp)
{
    ...
    // Spoomy address
    CSpoomyAddress address = CSpoomyAddress(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Spoomy address");
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw std::runtime_error(
            ...
            "\nDEPRECATED (use sendtoaddress). Sent an amount from an account to a spoomy address."
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
    ...
    std::string strAccount = AccountFromValue(params[0]);
    CSpoomyAddress address(params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Spoomy address");
    CAmount nAmount = AmountFromValue(params[2]);
    if (nAmount <= 0)
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    ...
    std::set<CSpoomyAddress> setAddress;
    std::vector<CRecipient> vecSend;
    ...
    {
        CSpoomyAddress address(name_);
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Spoomy address: ")+name_);
...
UniValue addmultisigaddress(const UniValue& params, bool fHelp)
{
    ...
    return CSpoomyAddress(innerID).ToString();
}
...
UniValue ListReceived(const UniValue& params, bool fByAccounts)
{
    ...
    // Tally
    std::map<CSpoomyAddress, tallyitem> mapTally;
    for (std::map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        const CWalletTx& wtx = (*it).second;
    ...
    BOOST_FOREACH(const PAIRTYPE(CSpoomyAddress, CAddressBookData)& item, pwalletMain->mapAddressBook)
    {
        ...
        std::map<CSpoomyAddress, tallyitem>::iterator it = mapTally.find(address);
...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    ...
    StartShutdown();
    return "Wallet encrypted; Spoomy server stopping, restart to run with encrypted wallet. The keypool has been flushed and a new HD seed was generated (if you are using HD). You need to make a new backup.";
}
...
UniValue listunspent(const UniValue& params, bool fHelp)
{
    ...
    if (params.size() > 2) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            CSpoomyAddress address(input.get_str());
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Spoomy address: ")+input.get_str());
            if (setAddress.count(address))
    ...
    BOOST_FOREACH(const COutput& out, vecOutputs) {
        ...
        CTxDestination address;
        if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, address)) {
            entry.push_back(Pair("address", CSpoomyAddress(address).ToString()));
