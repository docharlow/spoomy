...
#endif
    GenerateSpoomys(false, 0, Params());
    StopNode();
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), ZUMY_CONF_FILENAME));
    if (mode == HMM_ZUMYD)
    {
#ifndef WIN32
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), ZUMY_PID_FILENAME));
#endif
    ...
    std::string debugCategories = "addrman, alert, bench, coindb, db, http, libevent, lock, mempool, mempoolrej, net, proxy, prune, rand, reindex, rpc, selectcoins, tor, zmq, "
                             "Spoomy (or specifically: privatesend, instantsend, masternode, spork, keepass, mnpayments, gobject)"; // Don't translate these and qt below
    if (mode == HMM_ZUMY_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Spoomy specific functionality (Masternodes, PrivateSend, InstantSend, Governance) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-privatesendamount=<n>", strprintf(_("Keep N SPY anonymized (default: %u)"), DEFAULT_PRIVATESEND_AMOUNT));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2018-%i The Credits Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Spoomy ecosystem created  2018-%i Launched by Lead Developer Harlow"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i Spoomy Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(_("This is experimental software. Please read our whitepaper fully on Spoomy.co before using wallet to understand our project ")) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that Spoomy is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Spoomy is shutting down."));
...
#endif
    // Make sure only a single Spoomy process is using the data directory.
    ...
    try {
        ...
        if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Spoomy is probably already running."), strDataDir));
    } catch(const boost::interprocess::interprocess_exception& e) {
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Spoomy is probably already running.") + " %s.", strDataDir, e.what()));
    }
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            if (nLoadWalletRet == DB_CORRUPT)
                strErrors << _("Error loading wallet.dat: Wallet is either corrupted OR you need to use your previous client to dump ALL private keys for ALL addresses."
                               ...
                               " Allow Spoomy to build a new wallet.dat, fully sync and then import the private keys to the newly created wallet") << "\n";
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Spoomy") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Spoomy to complete") << "\n";
    ...
    if(fMasterNode) {
        ...
        if(!strMasterNodePrivKey.empty()) {
            ...
            LogPrintf("  pubKeyMasternode: %s\n", CSpoomyAddress(activeMasternode.pubKeyMasternode.GetID()).ToString());
        } else {
    ...
    // ********************************************************* Step 11c: update block tip in Spoomy modules
    ...
    // ********************************************************* Step 11d: start spoomy-privatesend thread
    ...
    GenerateSpoomys(GetBoolArg("-gen", DEFAULT_GENERATE), GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
