...
BOOST_AUTO_TEST_CASE(rpc_wallet)
{
    ...
    UniValue r;
    CPubKey demoPubkey;
    CSpoomyAddress demoAddress;
    UniValue retValue;
    std::string strAccount = "walletDemoAccount";
    CSpoomyAddress setaccountDemoAddress;
    {
        LOCK(pwalletMain->cs_wallet);

        demoPubkey = pwalletMain->GenerateNewKey(0, false);
        demoAddress = CSpoomyAddress(CTxDestination(demoPubkey.GetID()));
        ...
        setaccountDemoAddress = CSpoomyAddress(CTxDestination(setaccountDemoPubkey.GetID()));
    }
    /*********************************
     *          setaccount
     *********************************/
    ...
    BOOST_CHECK(CSpoomyAddress(retValue.get_str()).Get() == demoAddress.Get());
    ...
    BOOST_CHECK(CSpoomyAddress(arr[0].get_str()).Get() == demoAddress.Get());
