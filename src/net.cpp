...
namespace {
    ...
    const int MAX_OUTBOUND_MASTERNODE_CONNECTIONS = 64;
...
CNode* ConnectNode(CAddress addrConnect, const char *pszDest, bool fConnectToMasternode)
{
    if (pszDest == NULL) {
        ...
        if (pnode)
        {
            // we have existing connection to this node but it was not a connection to Masternodes,
...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Spoomy users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        std::string strDesc = "Spoomy " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, std::string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Spoomy is probably already running."), addrBind.ToString());
        else
