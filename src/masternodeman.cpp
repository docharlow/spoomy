...
void CMasternodeMan::AskForMN(CNode* pnode, const CTxIn &vin)
{
    ...
    mWeAskedForMasternodeListEntry[vin.prevout][pnode->addr] = GetTime() + SSEG_UPDATE_SECONDS;
...
/* Only IPv4 Masternodes are allowed, saving this for later
...
void CMasternodeMan::SsegUpdate(CNode* pnode)
{
    LOCK(cs);
    ...
    if(Params().NetworkIDString() == CBaseChainParams::MAIN) {     
        if(!(pnode->addr.IsRFC1918() || pnode->addr.IsLocal())) {
            ...
            if(it != mWeAskedForMasternodeList.end() && GetTime() < (*it).second) {
                LogPrintf("CMasternodeMan::SsegUpdate -- we already asked %s for the list; skipping...\n", pnode->addr.ToString());
                return;
            }
        }
    }      
    ...
    LogPrint("Masternode", "CMasternodeMan::SsegUpdate -- asked %s for the list\n", pnode->addr.ToString());
}
...
void CMasternodeMan::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == NetMsgType::MNANNOUNCE) { //Masternode Broadcast
    ...
    } else if (strCommand == NetMsgType::SSEG) { //Get Masternode list or specific entry
        ...
        LogPrint("Masternode", "SSEG -- Masternode list, Masternode=%s\n", vin.prevout.ToStringShort());
        ...
        if(vin == CTxIn()) { //only should ask for this once
            ...
            // This is to prevent unnecessary banning of Masternodes whilst the network is in its infancy
            if(!isLocal && Params().NetworkIDString() == CBaseChainParams::MAIN && nMnCount > 200) {
        ...
        BOOST_FOREACH(CMasternode& mn, vMasternodes) {
            ...
            LogPrint("Masternode", "SSEG -- Sending Masternode entry: Masternode=%s  addr=%s\n", mn.vin.prevout.ToStringShort(), mn.addr.ToString());
            ...
            if (vin == mn.vin) {
                LogPrintf("SSEG -- Sent 1 Masternode inv to peer %d\n", pfrom->id);
                return;
            }
        }
        ...
        if(vin == CTxIn()) {
            ...
            LogPrintf("SSEG -- Sent %d Masternode invs to peer %d\n", nInvCount, pfrom->id);
            return;
        }
        // smth weird happen - someone asked us for vin we have no idea about?
        LogPrint("Masternode", "SSEG -- No invs sent to peer %d\n", pfrom->id);
...
// Verification of Masternode via unique direct requests.
...
void CMasternodeMan::ProcessVerifyBroadcast(CNode* pnode, const CMasternodeVerification& mnv)
{
    ...
    if(nRank < MAX_POSE_RANK) {
        LogPrint("Masternode", "MasternodeMan::ProcessVerifyBroadcast -- Masternode is not in top %d, current rank %d, peer=%d\n",
...

    {
        ...
        if(!pmn2) {
            LogPrintf("CMasternodeMan::ProcessVerifyBroadcast -- can't find Masternode %s\n", mnv.vin2.prevout.ToStringShort());
            return;
        }
