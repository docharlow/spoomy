...
UniValue getpoolinfo(const UniValue& params, bool fHelp)
{
    ...
    if (privateSendPool.pSubmittedToMasternode) {
        obj.push_back(Pair("outpoint",      privateSendPool.pSubmittedToMasternode->vin.prevout.ToStringShort()));
        obj.push_back(Pair("addr",          privateSendPool.pSubmittedToMasternode->addr.ToString()));
    }
...
UniValue masternode(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp  ||
        (strCommand != "start" && strCommand != "start-alias" && strCommand != "start-all" && strCommand != "start-missing" &&
         ...
         strCommand != "connect" && strCommand != "outputs" && strCommand != "status"))
            throw std::runtime_error(
                "masternode \"command\"...\n"
                "Set of commands to execute masternode-sync related actions\n"
                "\nArguments:\n"
                "1. \"command\"        (string or set of strings, required) The command to execute\n"
                "\nAvailable commands:\n"
                ...
                "  start        - Start local Hot Masternode configured in spoomy.conf\n"
    ...
    if (strCommand == "current" || strCommand == "winner")
    {
        ...
        obj.push_back(Pair("payee",         CSpoomyAddress(winner->pubKeyCollateralAddress.GetID()).ToString()));
    ...
    if (strCommand == "genkey")
    {
        ...
        return CSpoomySecret(secret).ToString();
    }
    ...
    if (strCommand == "outputs") {
        ...
        pwalletMain->AvailableCoins(vPossibleCoins, true, NULL, false, ONLY_500);
    ...
    if (strCommand == "status")
    {
        ...
        if(mnodeman.Get(activeMasternode.vin, mn)) {
            mnObj.push_back(Pair("payee", CSpoomyAddress(mn.pubKeyCollateralAddress.GetID()).ToString()));
        }
...
UniValue masternodelist(const UniValue& params, bool fHelp)
{
                ...
                strMode != "protocol" && strMode != "payee" && strMode != "rank" && strMode != "status"))
    {
        throw std::runtime_error(
                ...
                "  payee          - Print Spoomy address associated with a Masternode (can be additionally filtered,\n"
                "                   partial match)\n"
        ...
        BOOST_FOREACH(CMasternode& mn, vMasternodes) {
            ...
            if (strMode == "activeseconds") {
                ...
                streamFull << std::setw(18) <<
                               mn.GetStatus() << " " <<
                               mn.nProtocolVersion << " " <<
                               CSpoomyAddress(mn.pubKeyCollateralAddress.GetID()).ToString() << " " <<
                ...
                CSpoomyAddress address(mn.pubKeyCollateralAddress.GetID());
    ...
    if (strCommand == "decode")
    {
        ...
        BOOST_FOREACH(CMasternodeBroadcast& mnb, vecMnb) {
            ...
            if(mnb.CheckSignature(nDos)) {
                ...
                resultObj.push_back(Pair("pubKeyCollateralAddress", CSpoomyAddress(mnb.pubKeyCollateralAddress.GetID()).ToString()));
                resultObj.push_back(Pair("pubKeyMasternode", CSpoomyAddress(mnb.pubKeyMasternode.GetID()).ToString()));
                resultObj.push_back(Pair("vchSig", EncodeBase64(&mnb.vchSig[0], mnb.vchSig.size())));
                resultObj.push_back(Pair("sigTime", mnb.sigTime));
                resultObj.push_back(Pair("protocolVersion", mnb.nProtocolVersion));
                resultObj.push_back(Pair("nLastSsq", mnb.nLastSsq));
