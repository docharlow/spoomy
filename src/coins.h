...
public:
    ...
    size_t SpoomyMemoryUsage() const {
        size_t ret = memusage::SpoomyUsage(vout);
        BOOST_FOREACH(const CTxOut &out, vout) {
            ret += RecursiveSpoomyUsage(out.scriptPubKey);
        }
        return ret;
    }
};
...
protected:
    ...
    /* Cached spoomy memory usage for the inner CCoins objects. */
...
public:
    ...
    /** 
     * Amount of spoomy coming in to a transaction
