...
void CActiveMasternode::ManageStateInitial()
{
    ...
    if (pnode->addr.IsIPv6()) {
        // listen option is probably overwritten by smth else, no good
        LogPrintf("Masternodes cannot use IPv6, you must use IPv4 for Masternode connectivity.");
        return;
        }
    }
    ...
    if(Params().NetworkIDString() == CBaseChainParams::MAIN) {
        if(service.GetPort() != mainnetDefaultPort) {
            nState = ACTIVE_MASTERNODE_NOT_CAPABLE;
            strNotCapableReason = strprintf("Invalid port: %u - only 32000 is supported on mainnet.", service.GetPort());
    ...
    if(Params().NetworkIDString() != CBaseChainParams::MAIN) {
        if(service.GetPort() == mainnetDefaultPort) {
            nState = ACTIVE_MASTERNODE_NOT_CAPABLE;
            strNotCapableReason = strprintf("Invalid port: %u - 32000 is only supported on mainnet.", service.GetPort());
            LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", strNotCapableReason);
            return;
        }
    }
    ...
    if(pwalletMain->GetBalance() < 2000*COIN) {
        LogPrintf("CActiveMasternode::ManageStateInitial -- %s: Wallet balance is < 2000 SPY\n", GetStateString());
        return;
    }
...
void CActiveMasternode::ManageStateLocal()
{
    ...
    if(pwalletMain->GetMasternodeVinAndKeys(vin, pubKeyCollateral, keyCollateral)) {
        ...
        if(!CMasternodeBroadcast::Create(vin, service, keyCollateral, pubKeyCollateral, keyMasternode, pubKeyMasternode, strError, mnb)) {
            nState = ACTIVE_MASTERNODE_NOT_CAPABLE;
            strNotCapableReason = "Error creating Masternode broadcast: " + strError;
            LogPrintf("CActiveMasternode::ManageStateLocal -- %s: %s\n", GetStateString(), strNotCapableReason);
            return;
        }
