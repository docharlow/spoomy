...
const std::string SpoomyGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString SpoomyGUI::DEFAULT_WALLET = "~Default";

SpoomyGUI::SpoomyGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
...
void SpoomyGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Spoomy address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and spoomy: URIs)"));
    ...
    aboutAction = new QAction(QIcon(":/icons/" + theme + "/about"), tr("&About Spoomy"), this);
    aboutAction->setStatusTip(tr("Show information about Spoomy"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for Spoomy"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Spoomy addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/" + theme + "/transaction_0"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Spoomy addresses"));
    ...
    openSNConfEditorAction = new QAction(QIcon(":/icons/" + theme + "/edit"), tr("Open &Masternode Configuration File"), this);
    openSNConfEditorAction->setStatusTip(tr("Open Masternode configuration file"));    
    ...
    openAction->setStatusTip(tr("Open a spoomy: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the Spoomy help message to get a list with possible Spoomy command-line options"));
...
void SpoomyGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool SpoomyGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool SpoomyGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void SpoomyGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void SpoomyGUI::setWalletActionsEnabled(bool enabled)
{
...
void SpoomyGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
    trayIcon = new QSystemTrayIcon(this);
    QString toolTip = tr("Spoomy client") + " " + networkStyle->getTitleAddText();
...
void SpoomyGUI::createIconMenu(QMenu *pmenu)
{
...
#ifndef Q_OS_MAC
void SpoomyGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void SpoomyGUI::showInfo()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_INFO);
    showRPCConsole();
}

void SpoomyGUI::showConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showRPCConsole();
}

void SpoomyGUI::showGraph()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_GRAPH);
    showRPCConsole();
}

void SpoomyGUI::showPeers()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_PEERS);
    showRPCConsole();
}

void SpoomyGUI::showRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showRPCConsole();
}

void SpoomyGUI::showConfEditor()
{
    GUIUtil::openConfigfile();
}

void SpoomyGUI::showSNConfEditor()
{
    GUIUtil::openSNConfigfile();
}

void SpoomyGUI::showBackups()
{
    GUIUtil::showBackups();
}

void SpoomyGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

void SpoomyGUI::showPrivateSendHelpClicked()
{
    if(!clientModel)
        return;
...
void SpoomyGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void SpoomyGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void SpoomyGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void SpoomyGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void SpoomyGUI::gotoMasternodePage()
{
    masternodeAction->setChecked(true);
    if (walletFrame) walletFrame->gotoMasternodePage();
}

void SpoomyGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void SpoomyGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void SpoomyGUI::setNumConnections(int count)
{
    QString icon;
    QString theme = GUIUtil::getThemeName();
    switch(count)
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Spoomy network", "", count));
}

void SpoomyGUI::updateHeadersSyncProgressLabel()
{
...
void SpoomyGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void SpoomyGUI::setAdditionalDataSyncProgress(double nSyncProgress)
{
    if(!clientModel)
        return;
...
void SpoomyGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Spoomy"); // default title
...
void SpoomyGUI::changeEvent(QEvent *e)
{
...
void SpoomyGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void SpoomyGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void SpoomyGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(SpoomyUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void SpoomyGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void SpoomyGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool SpoomyGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool SpoomyGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void SpoomyGUI::setHDStatus(int hdEnabled)
{
    QString theme = GUIUtil::getThemeName();
...
void SpoomyGUI::setEncryptionStatus(int status)
{
    QString theme = GUIUtil::getThemeName();
    switch(status)
    {
...
void SpoomyGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void SpoomyGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void SpoomyGUI::detectShutdown()
{
...
void SpoomyGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void SpoomyGUI::showModalOverlay()
{
    if (modalOverlay)
        modalOverlay->showHide(false, true);
}

static bool ThreadSafeMessageBox(SpoomyGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void SpoomyGUI::subscribeToCoreSignals()
{
    // Connect signals to client
    uiInterface.ThreadSafeMessageBox.connect(boost::bind(ThreadSafeMessageBox, this, _1, _2, _3));
}

void SpoomyGUI::unsubscribeFromCoreSignals()
{
...
void SpoomyGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<SpoomyUnits::Unit> units = SpoomyUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const SpoomyUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(SpoomyUnits::name(unit)));
    }
...
{
    menu = new QMenu();
    Q_FOREACH(SpoomyUnits::Unit u, SpoomyUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(SpoomyUnits::name(u)), this);
...
{
    setPixmap(QIcon(":/icons/light/unit_" + SpoomyUnits::id(newUnits)).pixmap(33,STATUSBAR_ICONSIZE));
}
