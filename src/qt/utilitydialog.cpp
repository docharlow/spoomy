    ...
    ui(new Ui::HelpMessageDialog)
{
    ...
    QString version = tr("Spoomy") + " " + tr("version") + " " + QString::fromStdString(FormatFullVersion());
    ...
    if (helpMode == about)
    {
        setWindowTitle(tr("About Spoomy"));
        ...
        QString header = tr("Usage:") + "\n" +
            "  spoomy-qt [" + tr("command-line options") + "]                     " + "\n";
        ...
        std::string strUsage = HelpMessage(HMM_ZUMY_QT);
        ...
        if (showDebug) {
            strUsage += HelpMessageOpt("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", SpoomyGUI::DEFAULT_UIPLATFORM));
        }
...
All the Spoomy in your wallet is comprised of different \"inputs\" which you can think of as separate, discrete coins.<br> \
...
These denominations are 0.01 SPY, 0.1 SPY, 1 SPY, 10 SPY -- sort of like the paper money you use every day.</li> \
    ...
    QWidget(parent, f)
{
    ...
    layout->addWidget(new QLabel(
        tr("Spoomy is shutting down...") + "<br /><br />" +
        tr("Do not shut down the computer until this window disappears.")));
    setLayout(layout);
}

void ShutdownWindow::showShutdownWindow(SpoomyGUI *window)
{
    if (!window)
        return;
