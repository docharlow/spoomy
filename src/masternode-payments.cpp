...
*
*   Why is this needed?
*   - In Spoomy some blocks are superblocks, which output much higher amounts of coins
...
bool IsBlockPayeeValid(const CTransaction& txNew, int nBlockHeight, CAmount blockReward)
{
    ...
    if(nBlockHeight < consensusParams.nSuperblockStartBlock) {
        ...
        // Superblocks are disabled for Spoomy.
        int nOffset = nBlockHeight % consensusParams.nBudgetPaymentsCycleBlocks;
        if(nBlockHeight >= consensusParams.nBudgetPaymentsStartBlock &&
            nOffset < consensusParams.nBudgetPaymentsWindowBlocks) {
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, CAmount nFees, CTxOut& txoutMasternodeRet)
{
    ...
    if (chainActive.Height() <= Params().GetConsensus().nMasternodePaymentsStartBlock) {
            if (fDebug)
                LogPrintf("CreateNewBlock: No Masternode payments prior to block 101\n");
            hasPayment = false;
    }
    ...
    CAmount devPayment = 0.2 * COIN; // Developer Gov fund amount
    ...
    if (nNextHeight > Params().GetConsensus().nDevPhaseTotalBlocks && nNextHeight <= Params().GetConsensus().nPhase5LastBlock) {
        ...
        std::string strDevAddress = "ZUMYD3VX7ZcbBpdZMtMNioAAKQjKguKWmd"; // Address Payment of 0.2 SPY
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f SPY\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
