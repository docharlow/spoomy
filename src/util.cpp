...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "spoomy" is a composite category enabling all Spoomy-related debug output
            if(ptrCategory->count(std::string("spoomy"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Spoomy
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Spoomy
    // Mac: ~/Library/Application Support/Spoomy
    // Unix: ~/.spoomy
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Spoomy";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Spoomy";
#else
    // Unix
    return pathRet / ".spoomy";
#endif
#endif
}
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", ZUMY_CONF_FILENAME));
                    ...
                    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create spoomy.conf if it does not exist
        FILE* configFile = fopen(GetConfigFile().string().c_str(), "a");
        if (configFile != NULL) {
            // Write spoomy.conf file with random username and password.
            WriteConfigFile(configFile);
            // New spoomy.conf file written, now read it.
        }
    }
    ...
    {
        // Don't overwrite existing settings so command line settings override spoomy.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", ZUMY_PID_FILENAME));
