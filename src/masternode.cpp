...
bool CMasternodeBroadcast::Create(std::string strService, std::string strKeyMasternode, std::string strTxHash, std::string strOutputIndex, std::string& strErrorRet, CMasternodeBroadcast &mnbRet, bool fOffline)
{
    ...
    if (pnode->addr.IsIPv6()) {
        LogPrintf("Invalid protocol for Masternode, only IPv4 is supported.");
        return false;
        }
    }
...
bool CMasternodeBroadcast::Create(CTxIn txin, CService service, CKey keyCollateralAddressNew, CPubKey pubKeyCollateralAddressNew, CKey keyMasternodeNew, CPubKey pubKeyMasternodeNew, std::string &strErrorRet, CMasternodeBroadcast &mnbRet)
{
    ...
    LogPrint("Masternode", "CMasternodeBroadcast::Create -- pubKeyCollateralAddressNew = %s, pubKeyMasternodeNew.GetID() = %s\n",
             CSpoomyAddress(pubKeyCollateralAddressNew.GetID()).ToString(),
...
{
    // we are a Masternode with the same vin (i.e. already activated) and this mnb is ours (matches our Masternodes privkey)
...

    {
        ...
        if(coins.vout[vin.prevout.n].nValue != 2000 * COIN) {
            LogPrint("Masternode", "CMasternodeBroadcast::CheckOutpoint -- Masternode UTXO should have 2000 SPY, Masternode=%s\n", vin.prevout.ToStringShort());
            return false;
        }
    ...
    // should be at least not earlier than block when 2000 SPY tx got nMasternodeMinimumConfirmations
...
bool CMasternodePing::SimpleCheck(int& nDos)
{
...

    {
        ...
        if (mi == mapBlockIndex.end()) {
            LogPrint("Masternode", "MasternodePing::SimpleCheck -- Masternode ping is invalid, unknown block hash: Masternode=%s blockHash=%s\n", vin.prevout.ToStringShort(), blockHash.ToString());
