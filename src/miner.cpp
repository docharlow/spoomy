...
void static SpoomyMiner(const CChainParams& chainparams)
{
    LogPrintf("SpoomyMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("spoomy-miner");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("SpoomyMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("SpoomyMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("SpoomyMiner -- terminated\n");
    ...
    {
        LogPrintf("SpoomyMiner -- runtime error: %s\n", e.what());
        #ifdef __AVX2__
        WolfArgon2dFreeCtx(Ctx);
        #endif
        return;
...
void GenerateSpoomys(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SpoomyMiner, boost::cref(chainparams)));
}
