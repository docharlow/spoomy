    ...
    spendsCoinbase(_spendsCoinbase), sigOpCount(_sigOps), lockPoints(lp)
{
    ...
    nUsageSize = RecursiveSpoomyUsage(tx);
...
bool CTxMemPool::addUnchecked(const uint256& hash, const CTxMemPoolEntry &entry, setEntries &setAncestors, bool fCurrentEstimate)
{
    ...
    // further updated.)
    cachedInnerUsage += entry.SpoomyMemoryUsage();
...
void CTxMemPool::removeUnchecked(txiter it)
{
    ...
    cachedInnerUsage -= it->SpoomyMemoryUsage();
    cachedInnerUsage -= memusage::SpoomyUsage(mapLinks[it].parents) + memusage::SpoomyUsage(mapLinks[it].children);
    mapLinks.erase(it);
    mapTx.erase(it);
    nTransactionsUpdated++;
...
void CTxMemPool::check(const CCoinsViewCache *pcoins) const
{
    ...
    for (indexed_transaction_set::const_iterator it = mapTx.begin(); it != mapTx.end(); it++) {
        ...
        innerUsage += it->SpoomyMemoryUsage();
        const CTransaction& tx = it->GetTx();
        txlinksMap::const_iterator linksiter = mapLinks.find(it);
        assert(linksiter != mapLinks.end());
        const TxLinks &links = linksiter->second;
        innerUsage += memusage::SpoomyUsage(links.parents) + memusage::SpoomyUsage(links.children);
...
size_t CTxMemPool::SpoomyMemoryUsage() const {
    LOCK(cs);
    // Estimate the overhead of mapTx to be 12 pointers + an allocation, as no exact formula for boost::multi_index_contained is implemented.
    return memusage::MallocUsage(sizeof(CTxMemPoolEntry) + 12 * sizeof(void*)) * mapTx.size() + memusage::SpoomyUsage(mapNextTx) + memusage::SpoomyUsage(mapDeltas) + memusage::SpoomyUsage(mapLinks) + cachedInnerUsage;
}
...
void CTxMemPool::UpdateChild(txiter entry, txiter child, bool add)
{
    ...
    if (add && mapLinks[entry].children.insert(child).second) {
        cachedInnerUsage += memusage::IncrementalSpoomyUsage(s);
...
CFeeRate CTxMemPool::GetMinFee(size_t sizelimit) const {
    ...
    if (time > lastRollingFeeUpdate + 10) {
        ...
        if (SpoomyMemoryUsage() < sizelimit / 4)
            halflife /= 4;
        else if (SpoomyMemoryUsage() < sizelimit / 2)
            halflife /= 2;
...
void CTxMemPool::TrimToSize(size_t sizelimit, std::vector<uint256>* pvNoSpendsRemaining) {
    ...
    while (!mapTx.empty() && SpoomyMemoryUsage() > sizelimit) {
