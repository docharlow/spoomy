...
#if defined(NDEBUG)
# error "Spoomy cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Spoomy Signed Message:\n";
...
int GetInputAgeIX(const uint256 &nTXHash, const CTxIn &txin)
{
    ...
    if (nResult < 10 && instantsend.IsLockedInstantSendTransaction(nTXHash))
...
CAmount GetPoWBlockPayment(const int& nHeight, CAmount nFees)
{
    ...
    if (chainActive.Height() == 0) {
        CAmount nSubsidy = 640000 * COIN;
        LogPrint("superblock creation", "GetPoWBlockPayment() : create=%s nSubsidy=%d\n", FormatMoney(nSubsidy), nSubsidy);
        return nSubsidy;
	}
    else if (chainActive.Height() == 1) {
        CAmount nSubsidy = 5 * COIN;
        LogPrint("superblock creation", "GetPoWBlockPayment() : create=%s nSubsidy=%d\n", FormatMoney(nSubsidy), nSubsidy);
        return nSubsidy;
    }
    else if (chainActive.Height() >= 2 && nNextHeight <= Params().GetConsensus().nSpoomyBeginsBlock) {
        nIntPoWReward = 0.05* COIN;
        LogPrint("creation", "GetPoWBlockPayment() : create=%s PoW Reward=%d\n", FormatMoney(nIntPoWReward), nIntPoWReward);
        return nIntPoWReward;
    }
    else if (chainActive.Height() >= 5 && nNextHeight <= Params().GetConsensus().nPhase1LastBlock) {
...
bool IsFundRewardValid(const CTransaction& txNew, CAmount fundReward) {
    ...
    CSpoomyAddress intAddress(strDevAddress.c_str());
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, const bool fWriteNames)
{
    ...
    // SPY : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (nNextHeight > Params().GetConsensus().nPhase1LastBlock && nNextHeight <= Params().GetConsensus().nPhase3LastBlock) {
        ...
        return state.DoS(0, error("ConnectBlock(SPY): didn't pay the Development Fund"), REJECT_INVALID, "bad-cb-amount");
        }
    }
    ...
    if(!IsBlockValueValid(block, pindex->nHeight, nExpectedBlockValue, strError)){
        return state.DoS(0, error("ConnectBlock(SPY): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, nExpectedBlockValue)) {
        mapRejectedBlocks.insert(std::make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(SPY): couldn't find Masternode or Superblock payments"),
...
bool static FlushStateToDisk(CValidationState &state, FlushStateMode mode) {
    ...
    size_t cacheSize = pcoinsTip->SpoomyMemoryUsage();
...
void static UpdateTip(CBlockIndex *pindexNew) {
    ...
    LogPrintf("%s: new best=%s  height=%d  log2_work=%.8g  tx=%lu  date=%s progress=%f  cache=%.1fMiB(%utx)\n", __func__,
      ...
      Checkpoints::GuessVerificationProgress(chainParams.Checkpoints(), chainActive.Tip()), pcoinsTip->SpoomyMemoryUsage() * (1.0 / (1<<20)), pcoinsTip->GetCacheSize());
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // ZUMY : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
         ...
         BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(std::make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(SPY): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(SPY): spork is off, skipping transaction locking checks\n");
    }
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        if (nCheckLevel >= 3 && pindex == pindexState && (coins.SpoomyMemoryUsage() + pcoinsTip->SpoomyMemoryUsage()) <= nCoinCacheUsage) {
            bool fClean = true;
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /*
        Spoomy Related Inventory Messages
    ...
    else if (strCommand == NetMsgType::TX || strCommand == NetMsgType::PSTX || strCommand == NetMsgType::TXLOCKREQUEST)
    {
        ...
        if (!AlreadyHave(inv) && AcceptToMemoryPool(mempool, state, tx, true, &fMissingInputs))
        {
            ...
            LogPrint("mempool", "AcceptToMemoryPool: peer=%d: accepted %s (poolsz %u txn, %u kB)\n",
                pfrom->id,
                tx.GetHash().ToString(),
                mempool.size(), mempool.SpoomyMemoryUsage() / 1000);
...
bool SendMessages(CNode* pto)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        // In case there is a block that has been in flight from this peer for 2 + 0.2 * N times the block interval
